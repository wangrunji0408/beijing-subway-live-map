...
* `check_speed.py`：速度按**运行时间**计算——站牌上的分钟是到站时刻，列车停站 1 分钟，
  所以区间运行时间 = 相邻站时刻差 − 1 分钟（与地图动画一致）。用**官方站间距**（北京市轨道交通运营管理有限公司
...
- **终点站没有到达时刻表**（终点站的站牌只印反方向的发车时刻），所以运行图原本在倒数第二站就结束。
  现在：列车若已到达"最后一个有站牌的车站"，就继续运行到终点站（`MAX_TERMINUS_HOPS` 控制最多外推几站）；
  在中间折返的小交路车不受影响，仍然提前结束。**目前 104 个分组全部抵达终点站**（修复前 88 个未抵达）。
- **1 号线与八通线、4 号线与大兴线是同一条物理线路**，此前各自成组，导致两组都不覆盖对方区段
  （四惠东↔高碑店之间没有车）。现在按**物理线路**合并建图，1 号线贯通为 35 站的完整交路。

## 运营日筛选

地图的运营日按**星期几**选择（周一…周日），每一天自动汇总当天开行的所有时刻表类型：

| 星期 | 采用的时刻表 |
|---|---|
| 周一–周四 | 工作日 / 周一至周四+双休日 / 平日（大兴机场线） |
| 周五 | 工作日 / 周五（首都机场线）/ 重点保障（大兴机场线） |
| 周六 | 双休日 / 周一至周四+双休日 / 平日 |
| 周日 | 双休日 / 周一至周四+双休日 / 平日 / 周日延长（大兴机场线） |
...
侧栏「实时统计」下方有一张折线图，显示当前运营日、当前显示线路组合下**全天每 5 分钟的在
线列车数**（4:00–25:00），红线标出当前时刻并给出读数与全天峰值。切换运营日或隐藏线路会
立即重算。实现见 `web/index.html` 的 `trainSeries()` / `drawHist()`。

## 线路高亮

点击侧栏的线路标签不再隐藏线路，而是**高亮**它：该线路保持全色、其余线路淡出到 16%，
同时「在线列车数 · 全天变化」折线图只统计被高亮的线路。支持多选（再点一下取消），
全部取消即恢复显示全网。

## 区间用时双向对称检查

`check_speed.py` 还会比较同一对相邻站的 A→B 与 B→A 用时——列车两个方向跑同一段路，
用时应当基本相同。差异 ≥ `--sym-tol`（默认 1.5 分钟）即列为异常。当前有 44 对不对称，
集中在机场线等站点稀疏的线路（终点站无到达时刻表，末段用几何里程外推）以及个别
反向时刻表含噪声数据的区间（如 10 号线 `火器营→巴沟` 校准成 5 分钟，反方向只要 2–3 分钟）。
...
上述区间已修正为 3 分钟（50 km/h），`马泉营→崔各庄` 从 7 分钟修正为 3 分钟。
...
- 去程（开往2号航站楼）：三元桥 → **3号航站楼 → 2号航站楼**（终点）
- 回程（开往北新桥）：**3号航站楼 → 2号航站楼 → 三元桥** —— 从 2号航站楼**直达三元桥，不再经过 3号航站楼**

OSM 关系里把 2号/3号 航站楼的顺序写反了，已用 `infer_diagram.ORDER_OVERRIDE` 修正；
官方站间距表用的是 `T3航站楼`/`T2航站楼`，与站名 `首都机场3号航站楼` 不一致，已补别名；
环线回程段（T2→三元桥）站间距表里没有，用 OSM 直线距离 ×1.15 兜底，使校准时仍有物理约束。

### 同一站的重复站牌

首都机场线每一站都贴了两张牌：**「发车时刻表」**与**「列车时刻表」**，方向与运营日相同、
内容几乎一致（例如三元桥两张表首班都是 06:55）。此前把同一站同一方向同一运营日的多张表
做**并集**，造成重复计数（三元桥 88 + 90 → 178 班）。

现在 `infer_diagram` 在合并时按"**来自不同站牌、时刻相差 ≤1 分钟**"视为同一班车只保留一条；
同一张牌内部相隔 1 分钟的两班车是真实存在的高峰加密，予以保留。
首都机场线各站班次因此从 ~170 回到真实的 ~155。
